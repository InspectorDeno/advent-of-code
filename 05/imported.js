const inputs = (await Bun.file('./05/input.txt').text()).trim().split('\n\n')
...
seeds.forEach((seed) => {
  let x = process(
    process(process(process(process(process(process(seed, c1), c2), c3), c4), c5), c6),
    c7
  )
...
for (let i = 0; i < seeds.length; i += 2) {
  for (let j = 0; j < seeds[i + 1]; j++) {
    let x = process(
      process(process(process(process(process(process(seeds[i] + j, c1), c2), c3), c4), c5), c6),
      c7
    )
